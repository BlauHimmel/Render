...
void Image::SaveTGA(const char *filename) const {
	...
	for (int y = height - 1; y >= 0; y--) {
		for (int x = 0; x < width; x++) {
			...
			WriteByte(file, ClampColorComponent(v._b));
			WriteByte(file, ClampColorComponent(v._g));
			WriteByte(file, ClampColorComponent(v._r));
		}
	}
	fclose(file);
}
...
Image* Image::LoadTGA(const char *filename) {
	...
	for (int y = height - 1; y >= 0; y--) {
		for (int x = 0; x < width; x++) {
			...
			g = ReadByte(file);
			r = ReadByte(file);
			Color color((_FLOAT)r / 255.0f, (_FLOAT)g / 255.0f, (_FLOAT)b / 255.0f);
...
void Image::SavePPM(const char *filename) const {
	...
	for (int y = height - 1; y >= 0; y--) {
		for (int x = 0; x<width; x++) {
			Color v = GetPixel(x, y);
			fputc(ClampColorComponent(v._r), file);
			fputc(ClampColorComponent(v._g), file);
			fputc(ClampColorComponent(v._b), file);
		}
	}
	fclose(file);
}
